...
impl From<Row> for RdbcRow {
    fn from(row: Row) -> Self {
        let mut orm_row = RdbcRow::new();
        let columns = row.columns();
        for col in columns {
            let col_name = col.name().to_string();
            orm_row.columns_mut().push(col_name.clone());
            ...
            match col_type.as_str() {
                ...
                _ => {
                    ...
                    orm_row.data_mut().insert(col_name, RdbcValue::Null);
                }
            }
        }
        orm_row
